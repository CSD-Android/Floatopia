 ...
 * 获取 View 的真实边界
 * */
val View.realBound get() = RectF(x, y, x + width, y + height)

val View.boundOnScreen: Rect get() {
    val location = locationOnScreen
    return Rect(location.first, location.second, location.first + width, location.second + height)
}
/**
 * 相对于 [Window] 的位置 (x, y)
 * */
